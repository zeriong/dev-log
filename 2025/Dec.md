...
- 사용자 관점에서 전체 애플리케이션이 의도한 대로 작동하는지 검증
- 브라우저 환경에서 실제 사용자 동작을 흉내 내어 테스트
...
```typescript
// 암묵적 대기 (비권장)
await page.waitForTimeout(3000); // 고정 시간 대기
```

#### \* 정리

- **E2E 테스트**: 사용자 관점에서 전체 애플리케이션 검증
- **유닛 테스트와 함께**: 상호 보완적으로 활용하여 안정성 극대화
- **중요 플로우 우선**: 비즈니스 핵심 로직에 집중
- **도구**: Cypress, Playwright 등 활용

<br>

#### 🔍 [ [매일메일 - 프론트엔드 E2E 테스트에 대해서 설명해주세요.](https://www.maeil-mail.kr/question/52) ]
...
- 클라이언트가 요청을 보내고 서버가 응답을 반환하는 방식으로 동작
...
- **HTTP에 보안을 강화한 버전**
- TLS/SSL 프로토콜로 데이터를 암호화하여 전송
- 중간자 공격 방어, 데이터 무결성 보장
- 보안 상 중요한 정보들을 안전하게 보호
...
1. **클라이언트-서버 분리**: 역할을 명확히 분리
2. **무상태성(Stateless)**: 서버는 클라이언트 상태를 저장하지 않음, 각 요청은 독립적
3. **일관된 인터페이스**: 고유한 URI로 리소스 식별, 일관된 인터페이스로 통신
4. **캐시 가능성**: 서버 응답 시간 개선을 위해 리소스 캐싱 지원
...
- **RESTful API**: REST 원칙을 따르는 API, 일관되고 예측 가능한 인터페이스

<br>

#### 🔍 [ [매일메일 - HTTP란 무엇인지 설명해 주세요.](https://www.maeil-mail.kr/question/101) ]
