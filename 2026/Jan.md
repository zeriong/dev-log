...
`지리적으로 분산된 여러 개의 서버를 이용해 웹 콘텐츠를 사용자에게 더 빠르게 전달하는 시스템이다.`

#### * 핵심 원리( 캐싱 )

- 사용자와 물리적으로 가장 가까운 곳에 위치한 **엣지 서버(Edtge Server)** 콘텐츠를 복사(캐싱)하여 저장한다.
- 사용자가 요청 시 원본 서버가 아닌 가까운 엣지 서버에서 데이터를 보내주어 지연 시간을 줄

#### * 주요 장점

- **속도 향상:** 사용자 응답 시간이 단축되어 웹사이트 로딩 속도가 빨라집니다.
- **부하 분산:** 원본 서버의 트래픽 부담을 줄여 서버 다운이나 성능 저하를 방지합니다.
- **보안 강화:** DDoS 공격 등 대규모 트래픽 공격을 완화하는 벽 역할을 합니다.

#### * 사용 사례

- 동영상 스트리밍(Netflix, YouTube), 고화질 이미지 전송, 대규모 파일 다운로드 등에 필수적으로 사용된다.

#### [🔍 [ Young - CDN이란 무엇인가 ](https://velog.io/@youngblue/CDN%EC%9D%B4%EB%9E%80-%EB%AC%B4%EC%97%87%EC%9D%B8%EA%B0%80) ]
...
- 클래스는 없이 호출 시 일반 함수로 동작하여 에러 발생 안 함 (위험요소)
- **클래스 (Class)**
